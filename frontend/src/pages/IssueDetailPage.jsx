...
export default function IssueDetailPage() {
  ...
  const { user } = useAuth();
  const [issue, setIssue] = useState(null); const [types, setTypes] = useState([]); const [assignees, setAssignees] = useState([]); const [comments, setComments] = useState([]); const [role, setRole] = useState('viewer'); const [error, setError] = useState(''); const [message, setMessage] = useState('');
  const load = useCallback(async () => { try { const issueResult = await issueApi.getIssue(issueKey); const [commentResult, typeResult, assigneeResult, projects] = await Promise.all([commentApi.listComments(issueKey), projectApi.listIssueTypes(issueResult.issue.project_id), projectApi.listAssignees(issueResult.issue.project_id), projectApi.listProjects()]); setIssue(issueResult.issue); setComments(commentResult.comments); setTypes(typeResult.issueTypes); setAssignees(assigneeResult.assignees); setRole(projects.projects.find((project) => project.id === issueResult.issue.project_id)?.project_role ?? 'viewer'); } catch (requestError) { setError(requestError.message); } }, [issueKey]);
  useEffect(() => { load(); }, [load]);
  ...
  return <div className="detail-grid"><section className="panel"><Link className="back-link" to={`/projects/${issue.project_id}/board`}>← Back to board</Link>{error && <p className="alert error">{error}</p>}{message && <p className="alert success">{message}</p>}<p className="eyebrow">{issue.issue_key} · {role}</p><h1>{issue.title}</h1><p className="issue-description">{issue.description || 'No description.'}</p><div className="issue-summary"><span className={`priority priority-${issue.priority}`}>{issue.priority}</span><span>Type #{issue.issue_type_id}</span><span>Status #{issue.status_id}</span></div><RoleGuard role={role} allow={['admin', 'member']}><h2>Edit issue</h2><IssueForm issueTypes={types} assignees={assignees} initialIssue={issue} onSubmit={edit} /></RoleGuard></section><section className="panel"><h2>Comments</h2><CommentList comments={comments} currentUserId={user.id} projectRole={role} onEdit={editComment} onDelete={deleteComment} /><RoleGuard role={role} allow={['admin', 'member']}><form className="comment-form" onSubmit={async (event) => { event.preventDefault(); const content = new FormData(event.currentTarget).get('content'); await addComment(content); event.currentTarget.reset(); }}><textarea name="content" required placeholder="Add a comment…" /><button className="button primary" type="submit">Comment</button></form></RoleGuard></section></div>;
}
