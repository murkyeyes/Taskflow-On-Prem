...
export default function ProjectSettingsPage() {
  ...
  if (role !== 'admin') return <div className="panel"><p className="alert error">Only project administrators can manage settings.</p><Link to={`/projects/${projectId}`}>Back to board</Link></div>;
  return <div className="settings-page"><div className="section-heading"><div><p className="eyebrow">{project.key} · administration</p><h1>{project.name} settings</h1></div><Link className="button subtle" to={`/projects/${projectId}`}>Back to board</Link></div>{error && <p className="alert error">{error}</p>}{message && <p className="alert success">{message}</p>}
    <section className="settings-grid">
      <div className="panel"><h2>Members</h2><form onSubmit={(event) => submit(event, (data) => projectApi.addMember(projectId, { userId: Number(data.get('userId')), projectRole: data.get('projectRole') }))} className="stack-form"><label>User ID<input name="userId" type="number" min="1" required /></label><label>Role<select name="projectRole" defaultValue="member"><option value="admin">Admin</option><option value="member">Member</option><option value="viewer">Viewer</option></select></label><button className="button primary">Add member</button></form><ul className="settings-list">{members.map((member) => <li key={member.user_id}><span>User #{member.user_id}</span><span className="muted">{member.project_role}</span><button className="link-button" onClick={() => projectApi.deleteMember(projectId, member.user_id).then(load)}>Remove</button></li>)}</ul></div>
