    ...
    <div className="workspace-main">
      ...
      <main className={`space-directory ${templatesOpen && canCreate ? 'with-templates' : ''}`}>
        <section className="space-directory-main">
          ...
          <div className="space-directory-table-wrap"><table className="space-directory-table"><thead><tr><th aria-label="Favorite">★</th><th><button type="button" onClick={() => setSortDirection((value) => value === 'asc' ? 'desc' : 'asc')}>Name <span>{sortDirection === 'asc' ? '↓' : '↑'}</span></button></th><th>Key</th><th>Type</th><th>Access</th><th aria-label="Space actions" /></tr></thead><tbody>{visibleSpaces.map((space) => { const template = templateByKey[space.template_key]; const isFavorite = favorites.includes(space.id); return <tr key={space.id}><td><button className={`favorite-button ${isFavorite ? 'active' : ''}`} type="button" aria-label={`${isFavorite ? 'Unstar' : 'Star'} ${space.name}`} onClick={() => toggleFavorite(space.id)}>{isFavorite ? '★' : '☆'}</button></td><td><Link className="space-name-cell" to={`/projects/${space.id}/summary`}><span className="project-avatar">{space.key.slice(0, 1)}</span><span><strong>{space.name}</strong><small>{space.description || 'No description provided.'}</small></span></Link></td><td>{space.key}</td><td><span>{template?.name || 'Kanban'}</span><small>{template?.category || 'Software development'}</small></td><td><span className="role-chip">{space.project_role === 'admin' ? 'Admin' : space.project_role === 'member' ? 'Member' : 'Viewer'}</span></td><td><details className="space-row-menu"><summary aria-label={`Actions for ${space.name}`}>•••</summary><div><Link to={`/projects/${space.id}/summary`}>Open Space</Link>{space.project_role === 'admin' && <Link to={`/projects/${space.id}/settings`}>Space settings</Link>}</div></details></td></tr>; })}{!visibleSpaces.length && <tr><td className="space-directory-empty" colSpan="6"><strong>{query || templateFilter !== 'all' ? 'No matching Spaces' : 'No Spaces assigned'}</strong><span>{query || templateFilter !== 'all' ? 'Try another search or template.' : 'Ask an Admin to assign you to a Space.'}</span></td></tr>}</tbody></table></div>
