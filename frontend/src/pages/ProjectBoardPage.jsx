...
import { filterIssuesByMonth, formatMonth, loadAllIssuePages, normalizeMonthKey } from '../utils/monthlyBacklog';
...
export default function ProjectBoardPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [projectResult, statusResult, typeResult, issueResult, projectList, assigneeResult, sprintResult] = await Promise.all([
        projectApi.getProject(projectId), projectApi.listWorkflowStatuses(projectId), projectApi.listIssueTypes(projectId), listAllIssues(projectId, { search: query, created_on: createdOn, completed_on: completedOn }), projectApi.listProjects(), projectApi.listAssignees(projectId, assigneeSearch), workspaceApi.listSprints(projectId),
      ]);
  ...
  const visibleIssues = useMemo(() => { const assigneeTerm = assigneeSearch.trim().toLowerCase(); return filterIssuesByMonth(issues, selectedMonth).filter((issue) => (!assigneeId ? (!assigneeTerm || (issue.assignee_name ?? '').toLowerCase().includes(assigneeTerm)) : issue.assignee_id === Number(assigneeId)) && (!priority || issue.priority === priority) && (!statusId || issue.status_id === Number(statusId))); }, [assigneeId, assigneeSearch, issues, priority, selectedMonth, statusId]);
