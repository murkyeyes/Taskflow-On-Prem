...
export default function WorkspaceShell() {
  ...
  return <div className="workspace">
    ...
    <div className="workspace-main">
      <div className="topbar"><Link className="mobile-brand" to="/projects">◆</Link><form className="global-search" onSubmit={searchSubmit}><span>⌕</span><input aria-label="Search project issues" placeholder="Search issues by key or title" value={search} onChange={(event) => setSearch(event.target.value)} />{searchResults.length > 0 && <div className="top-popover search-results">{searchResults.map((issue) => <Link key={issue.id} to={`/issues/${issue.issue_key}`} onClick={() => setSearch('')}><small>{issue.issue_key}</small><span>{issue.title}</span></Link>)}</div>}</form>{['admin', 'member'].includes(role) && <button className="button primary" type="button" onClick={() => { setCreateOpen((value) => !value); setActivityOpen(false); setAccountOpen(false); }}>＋ Create</button>}<div className="top-actions"><button className="top-action-button" type="button" title="Project activity" onClick={() => { setActivityOpen((value) => !value); setCreateOpen(false); setAccountOpen(false); }}>♢</button><Link className="top-action-button" to={`/projects/${projectId}/settings`} title="Project settings">⚙</Link><button className="avatar-button" type="button" title="Account menu" onClick={() => { setAccountOpen((value) => !value); setCreateOpen(false); setActivityOpen(false); }}>{user?.name?.slice(0, 1).toUpperCase()}</button></div>
        {createOpen && <div className="top-popover create-popover"><header><strong>Create issue</strong><button className="icon-button" type="button" onClick={() => setCreateOpen(false)}>×</button></header><IssueForm compact issueTypes={types} assignees={assignees} defaultStatusId={statuses.find((status) => status.is_default)?.id ?? statuses[0]?.id ?? ''} submitLabel="Create issue" onSubmit={createIssue} /></div>}
        {activityOpen && <div className="top-popover activity-popover"><strong>Project activity</strong><p className="muted">See recent issue changes, workload, and due dates.</p><Link className="button subtle" to={`/projects/${projectId}/summary`} onClick={() => setActivityOpen(false)}>Open Summary</Link></div>}
        {accountOpen && <div className="top-popover account-popover"><strong>{user?.name}</strong><small>{user?.email}</small><Link to="/projects" onClick={() => setAccountOpen(false)}>All projects</Link><button className="link-button" type="button" onClick={signOut}>Sign out</button></div>}
      </div>
